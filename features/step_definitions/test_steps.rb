...
Then(/^check file with selenium$/) do
  client = TestRail::APIClient.new('https://sguy.testrail.io/')
  client.user = 'kovtun65sergey@gmail.com'
  client.password = 'rO/jFTlT8SsjFFc63cNR'
  ...
  if driver.page_source().include? 'Untitled'
    ...
    r = client.send_post(
        'add_run/2',
        { name: 'rubyproject', include_all: TRUE }
    )
    puts r
